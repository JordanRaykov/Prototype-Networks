...
def CRBFtrain(data, class_labels, nl, niter, eta_s, N, d, basis_shape = 'gaussian', basis_parameters = None, use_pca = True, use_gplvm = True, basis_learning = None, inducing_num = None):  
    #Inputs:
...
toy = datasets.load_iris()

indices = np.arange(150)
np.random.shuffle(indices)

data_tot = toy.data[indices,:]
temp = toy.target.reshape(-1,1)[indices,:]
target_tot = np.zeros([150,3])
for i in range(150):
    target_tot[i, temp[i,:]] = 1

data_train = data_tot[:120,:]
target_train = target_tot[:120,:]
...
X, W, C, Beta, N = CRBFtrain(data_train, target_train, nl, niter, eta, N, d, basis_shape = 'zlogz', use_pca = use_pca, use_gplvm = use_gplvm)#, inducing_num = 30)
