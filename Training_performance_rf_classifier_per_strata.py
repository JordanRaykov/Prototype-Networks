# -*- coding: utf-8 -*-
"""
Created on Fri Oct 29 14:53:19 2021

@author: pmzyr
"""

def RFLOOCV(data, ids, outcomevar, idcolumn, numestimators=50, min_samples_split0 = 200, min_samples_leaf0 = 40, max_features0 = 'sqrt', max_depth0 = 10, bootstrap0 = False, fs=0.02):
    """
    ...
    rf = RandomForestClassifier(n_estimators = numestimators, max_depth = max_depth0, min_samples_split = min_samples_split0, min_samples_leaf = min_samples_leaf0, max_features = max_features0, bootstrap = bootstrap0, random_state = 0, class_weight="balanced")
...
data_features["Non-tremor/Tremor"][data_features["Non-tremor/Tremor"] == 0.1] = 0
...
for i in range(49):
    if i != 24:
        ...
        # Train data - all other people in dataframe
        data_train = data_filtered.drop(columns=["ID", "Medication_Intake","Prototype_ID","Activity_label","Non_tremor_activity_labels"])
        X_train = data_train.drop(columns=["Non-tremor/Tremor"])
        ...
        rf = RandomForestClassifier(n_estimators = 10, max_depth = 10, min_samples_split = 200, min_samples_leaf = 40, max_features = 'sqrt', bootstrap = False, random_state = 0, class_weight="balanced")
        rf.fit(X_train, y_train)
        
        prob_predictions_training = rf.predict_proba(X_train)
        ...
        prob_predictions_out_of_sample = rf.predict_proba(data_test)
        pred = rf.predict(data_test)
        
        #pred, prob_predictions_out_of_sample, label = RFLOOCV(data_test, i+1, outcomevar, idcolumn)
        ...
        data_curr = data_features[data_features["ID"] == i+1]
...
for i in range(8):
    tpr_proto_array[i,:] = TPR_proto_out_of_sample[i] 
...
for i in range(24):
    tnr_proto_array[i,:] = TNR_proto_out_of_sample[i]      
...
import pickle
with open('save_rf_TNR_per_nontremor_strata.pkl', 'wb') as f:
    pickle.dump(TNR_out_of_sample, f)
with open('save_rf_TPR_per_prototype_strata.pkl', 'wb') as f:
    pickle.dump(TPR_proto_out_of_sample, f)
with open('save_rf_AUC_per_prototype_strata.pkl', 'wb') as f:
    pickle.dump(AUC_proto_out_of_sample, f)
with open('save_rf_TNR_per_prototype_strata.pkl', 'wb') as f:
